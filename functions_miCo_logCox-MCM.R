...
# EM algorithm from the 'smcure' package
# input:  Time = follow-up times, Status = censoring indicator, X = covariates for the incidence, Z = covariates for the latency
#         gamma = initial value for gamma, beta = initial value for beta
#         enmax = maximum number of iterations, eps = tolerance for convergence
# output:  list containing the estimator of beta, gamma, the baseline survival function, convergence (distance between the old and the new estimators in the last iteration)

em =  function(Time,Status,X,Z,gamma,beta,emmax,eps)
  {     
    w = Status	
    n = length(Status)
    ...
    em = list(gamma=gamma, beta= beta,survival=s,tau=convergence)
  }



# SimexExtrapolation  (taken from the package 'miCoPTCM')
####################
# Performs the  Extrapolation part of the SIMEX algorithm
# Inputs:
...
{
  estim <- rep(NA,nbParam)
  estimNuExMY <- matrix(nrow=length(Nu),ncol=orderExtrap+nbParam)
  estimNuExMY[,1:nbParam] <- estimNui
  for(j in 1:orderExtrap)
  {
    estimNuExMY[,j+nbParam] <- Nu^j
  }
  temp1 <- paste("nu",1:orderExtrap,sep="")   #chr [1:2]  "nu1"  "nu2"
  temp2 <- paste(temp1,collapse="+")          # "nu1+nu2"
  temp3 <- paste("~",temp2,collapse="")       # "~nu1+nu2"
  colnames(estimNuExMY) <- c(variableNames,temp1)
  temp4 <- (-1)^(1+1:(orderExtrap+1))         # 1, -1, 1

  for (p in 1:nbParam) # For each parameter to be estimated
  {
    fm2 <- as.formula(paste(colnames(estimNuExMY)[p],temp3))     #beta ~ nu1 + nu2
...
print_results=function(x) 
{
  ...
  colnames(gamma) <- c("Estimate", "Std.Error", "Z value", 
                   "Pr(>|Z|)")
  gamma[, 2] <- x$gamma_sd
